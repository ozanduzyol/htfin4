...
async function dbGetAll() {
  try {
    const res = await fetch(`${SUPABASE_URL}/rest/v1/harcamalar?order=created_at.desc`, { headers: HEADERS });
    if (!res.ok) return [];
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          ...
                          <div style={{ fontSize: 11, color: C.muted, marginTop: 2 }}>
                            {c.label} {e.tarih ? "· " + new Date(e.tarih + "T00:00:00").toLocaleDateString("tr-TR") : ""}
                          </div>
                        </div>
